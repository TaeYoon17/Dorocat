...
final actor SyncedDatabase : Sendable {
    weak var coreDataClient: AnalyzeCoreDataClient!
    /// iCloud 컨테이너를 설정한다.
    static let container: CKContainer = CKContainer(identifier: "iCloud.com.tistory.arpple.Dorocat")
    var stateSerialization: CKSyncEngine.State.Serialization? {
        get {
            guard let data = UserDefaults.standard.data(forKey: "stateSerialization"),
        ...
        set {
            ...
            UserDefaults.standard.set(try? JSONEncoder().encode(newValue), forKey: "stateSerialization")
        }
    }
    ...
    var automaticallySync: Bool = false { // 일단 자동 싱크를 막는다. 추후에 수정해야할 필요 있음
        didSet {
    ...
    /// 생성자이다. 자동 싱크를 설정한다. dataURL은 없어도 된다.
...
extension SyncedDatabase : CKSyncEngineDelegate {
    ...
    func handleEvent(_ event: CKSyncEngine.Event, syncEngine: CKSyncEngine) async {
        ...
        switch event {
        case .stateUpdate(let event): // 상태 업데이트
            /// 저장 공간에 최신 상태가 무엇인지 저장하게 만들 필요가 있다.
            self.stateSerialization = stateSerialization
            print("[이게 왜 뭘까]", event.stateSerialization)
    ...
    func handleFetchedRecordZoneChanges(_ event: CKSyncEngine.Event.FetchedRecordZoneChanges) async {
        ...
        for modification in event.modifications {
            ...
            if findItem == nil {
                var item = TimerRecordItem(createdAt: .init(), duration: 0, session: .init(name: "Work"))
                item.mergeFromServerRecord(record)
